...
static int thermal_profile_setup(struct platform_device *device)
{
    ...
    /* Platform profile registration skipped due to kernel incompatibility */
    pr_info("Platform profile support disabled\n");
...
static int __init hp_wmi_init(void)
{
	...
	if (is_omen_v1_thermal_profile()) {
			err = /*omen_v1_register_powersource_event_handler() */ 0;
			if (err)
				goto err_unregister_device;
		}
